	...
	public static void main(String[] args) {
		...
		// initialize array of Employees
				Employee[] employees = { 
						...
						new Employee("Berihu", "Belachew", 2300, "Marketing") };
		...
		// 1) 
		System.out.println("Count Number of Employees whose last name starts with letter B and Print it "
				+list.stream()
				.filter(e -> e.getLastName().startsWith("B"))
				.count());

		// 2)
		System.out.println("Print the list of employees whose last name starts with letter B in sorted order:");
		...
		// 3)
		System.out.println(
				"Print employees whose last name starts with letter B and change the last name and first name to upper case");
		list.stream()
			.filter(e -> e.getLastName().startsWith("B"))
		...
		//Printing the Concatenated Stream
		empStream.forEach(System.out::println);
		
		System.out.println("The list of all employees but if the last name starts with B then capitalize it using List");

		List<Employee> empWithBList = list.stream()
				.filter(e -> e.getLastName().startsWith("B"))
				.map(e -> new Employee(e.getFirstName(), e.getLastName().toUpperCase(), e.getSalary(), e.getDepartment()))
				.collect(Collectors.toList());
		List<Employee> empWithoutBList = list.stream()
				.filter(e -> !e.getLastName().startsWith("B"))
				.map(e -> new Employee(e.getFirstName(), e.getLastName(), e.getSalary(), e.getDepartment()))
				.collect(Collectors.toList());
		...
		System.out.println("The avarage salary: " + 
				list.stream()
